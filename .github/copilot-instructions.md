...
- **Tidal API** - Tidal integration (requires Client ID and Client Secret)
- **Discord API** - Bot integration (requires Discord bot token)
- **ATProto** - Bluesky/ATProto integration for caching lookup results

### Aspire Orchestration
- **.NET Aspire** - Cloud-native orchestration for local development and production
- **Aspire Dashboard** - Built-in observability with traces, metrics, and logs
- **OpenTelemetry** - Distributed tracing and telemetry collection
- **Service Defaults** - Shared configuration for resilience, health checks, and OTLP

## Architecture Overview

BridgeBeats uses a **modular monolith** architecture with .NET Aspire orchestration:

### Project Dependency Graph
```
BridgeBeats.AppHost (Aspire orchestration)
└── BridgeBeats.Web (entry point)
    ├── BridgeBeats.ServiceDefaults (shared config)
    ├── BridgeBeats.Services (business logic)
    │   ├── BridgeBeats.Providers (music APIs)
    │   │   └── BridgeBeats.Contracts (DTOs)
    │   └── BridgeBeats.Infrastructure (data access)
    │       └── BridgeBeats.Contracts (DTOs)
    └── BridgeBeats.Infrastructure (data access)
```

### Design Principles
- **Compiler-enforced boundaries** - Projects reference only what they need
- **Single deployment artifact** - All components deployed together (current scale)
- **Future-ready for scaling** - Services layer designed for extraction to standalone workers
- **Aspire for all environments** - Local development and production use same orchestration
...
BridgeBeats/
├── src/
│   ├── BridgeBeats.AppHost/         # .NET Aspire orchestration (all environments)
│   ├── BridgeBeats.ServiceDefaults/ # Shared OTEL, health checks, resilience
│   ├── BridgeBeats.Contracts/       # DTOs, Interfaces, Enums, Constants (no dependencies)
│   ├── BridgeBeats.Providers/       # Music provider integrations (Apple Music, Spotify, Tidal)
│   ├── BridgeBeats.Infrastructure/  # DbContexts, Migrations, Entities, Caching, Identity, ATProto
│   ├── BridgeBeats.Services/        # Business logic (LinkResolver, Cards, Playlists)
│   └── BridgeBeats.Web/             # Web app, Controllers, Views, Middleware, Discord
├── Tests/
│   └── BridgeBeats.Tests/           # Unit, Integration, E2E tests
...
- **Dependency Injection**: All services registered in `BridgeBeats.Web/Configuration/StartupExtensions.cs`
...
### Security Guidelines
- **Never commit secrets** - Application credentials are encrypted in the database settings aggregate
- **Bootstrap infrastructure**: Redis credentials live in a Docker named volume; the standard Data
  Protection key ring lives in the persistent `./data/dp-keys` bind mount
- **Private key handling**: Apple and ATProto signing-key contents come from protected database settings
- **API authentication**: JWT for Apple Music, OAuth2 client credentials for Spotify

## Configuration and Environment

### Bootstrap Infrastructure
- The production SQLite path, Redis address, Data Protection paths, node number, and logging paths
  are fixed container conventions rather than operator configuration
- Redis credentials are initialized in a Docker named volume; ASP.NET Core manages its persistent
  Data Protection key ring in `./data/dp-keys`
- `REDIS_PASSWORD` is accepted only by the removable bootstrap command; otherwise it is generated
<!-- Application configuration follows. -->
### Application Settings
- AppHost loads the authoritative encrypted database snapshot and projects it to every process
- Stored updates require a controlled AppHost restart for activation
- Persisted models: `ApplicationSettingsValues` and `ApplicationSettingsSecrets`
...
- Entry point script: `containers/entrypoint.sh` (validates env vars and generates config)
