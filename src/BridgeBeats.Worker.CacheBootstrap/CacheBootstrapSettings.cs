...
/// to run the stale-cache refresh sweep, how many stale records each sweep enqueues, and the
/// per-leg enqueue pacing intervals.
/// </summary>
...
/// <param name="RefreshEnqueuePacing">The global per-leg drip interval: one leg is released every this duration.</param>
/// <param name="TidalRefreshMinInterval">The minimum gap between consecutive Tidal legs; burst capacity is zero.</param>
public sealed record CacheBootstrapSettings(
    ...
    TimeSpan RefreshEnqueuePacing,
    TimeSpan TidalRefreshMinInterval
);
