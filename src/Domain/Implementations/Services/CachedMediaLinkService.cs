...
namespace TuneBridge.Domain.Implementations.Services {
    ...
    public class CachedMediaLinkService : IMediaLinkService {
        ...
        public async IAsyncEnumerable<MediaLinkResult> GetInfoAsync( string content ) {
            ...
            Dictionary<string, HashSet<string>> staleEntries = new( );
            // Track fresh entries to associate new links later: recordUri -> list of cached input links
            Dictionary<string, HashSet<string>> freshEntries = new( );
            ...
            foreach (string link in extractedLinks) {
                ...
                (MediaLinkResult result, string recordUri, bool isStale)? cachedResult = await _cacheService.TryGetCachedResultAsync( link );
                if (cachedResult.HasValue) {
                    ...
                    if (cachedResult.Value.isStale) {
                        ...
                        _ = staleEntries[cachedResult.Value.recordUri].Add( link );
                    } else {
                        ...
                        _ = freshEntries[cachedResult.Value.recordUri].Add( link );
            ...
            List<string> nonCachedLinks = extractedLinks.Except( processedLinks ).ToList( );
            ...
            if (nonCachedLinks.Count > 0 || staleEntries.Count > 0) {
                ...
                await foreach (MediaLinkResult result in _innerService.GetInfoAsync( contentWithNonCachedLinks )) {
                    ...
                    List<string> resultInputLinks = result._inputLinks
