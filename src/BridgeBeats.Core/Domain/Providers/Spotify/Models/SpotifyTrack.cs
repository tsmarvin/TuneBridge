...
namespace BridgeBeats.Core.Domain.Providers.Spotify.Models {
    ...
    /// <summary>
    /// Deserialization target mirroring the Spotify Web API full <c>track</c> object. Carries the fields
    /// returned when a track is fetched directly (GET /tracks/{id}, GET /tracks?ids=) or as a search result,
    /// including the embedded album, external ids, and popularity that the simplified variant omits. The
    /// authoritative meaning of each field is the Spotify Web API object model; <c>[JsonPropertyName]</c>
    /// attributes map each property to its wire field. See <see cref="SpotifyTrackSimplified"/> for the
    /// simplified variant.
    /// </summary>
    public sealed class SpotifyTrack {

        /// <summary>Album the track appears on, as a simplified album object (no UPC); null when not returned. Maps to <c>album</c>.</summary>
        ...
        /// <summary>External identifiers for the track, such as the ISRC. Maps to <c>external_ids</c>.</summary>
        ...
        /// <summary>Spotify popularity score for the track, between 0 and 100 with 100 being most popular. Maps to <c>popularity</c>.</summary>
