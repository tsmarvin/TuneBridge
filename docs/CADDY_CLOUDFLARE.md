...
This document explains the custom Caddy Docker image with Cloudflare DNS plugin integration used by TuneBridge.

## Overview

TuneBridge uses a custom Caddy image that includes the Cloudflare DNS plugin. This enables automated certificate management for wildcard domains using DNS-01 ACME challenges, which is required for the ATProto PDS (Personal Data Server) subdomain wildcards.

## Why Cloudflare DNS Plugin?

The ATProto PDS requires wildcard DNS support (`*.pds.tunebridge.media`) to handle user-specific subdomains. Standard HTTP-01 ACME challenges cannot validate wildcard certificates, so we use DNS-01 challenges via the Cloudflare API.
...
```
<DOCKERHUB_USERNAME>/caddy-cloudflare-test:latest
```
...
`.github/workflows/publish-caddy-cloudflare-test.yml`
...
3. Verify DNS propagation:
   ```bash
   dig _acme-challenge.pds.tunebridge.media TXT
   ```
...
4. Try pulling manually:
   ```bash
   docker pull ${DOCKERHUB_USERNAME}/caddy-cloudflare-test:latest
   ```
