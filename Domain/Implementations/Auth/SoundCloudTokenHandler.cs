...
namespace TuneBridge.Domain.Implementations.Auth {
    ...
    /// <summary>
    /// Manages OAuth 2.0 client credentials flow for SoundCloud API authentication. Handles automatic
    ...
    /// </summary>
    /// <param name="auth">Pre-configured SoundCloud API credentials (client ID and secret in Base64 format).</param>
    /// <param name="factory">HTTP client factory configured with the SoundCloud auth endpoint.</param>
    ...
    public sealed class SoundCloudTokenHandler(
        ...
        /// Thrown if the SoundCloud auth server is unreachable or returns a non-success status code.
        /// </exception>
        ...
        private async Task<string> GetAppTokenAsync( ) {
            ...
            HttpClient client = factory.CreateClient("soundcloud-auth");
            using HttpRequestMessage req = new(HttpMethod.Post, "oauth2/token") {
            ...
            TokenResponse payload = await response.Content.ReadFromJsonAsync<TokenResponse>()
                         ?? throw new InvalidOperationException("Invalid soundcloud auth token response");
        ...
        /// This is used for the initial OAuth token request per the SoundCloud API specification.
        /// </summary>
        /// <returns>
        ...
        /// <summary>
        /// Creates a Bearer token authentication header for use in SoundCloud API requests. This method
        /// ensures the token is valid before returning, automatically refreshing expired tokens as needed.
        /// This is the primary public method used by SoundCloud API clients.
        /// </summary>
        /// <returns>
        ...
        /// </returns>
        /// <exception cref="HttpRequestException">
        /// Thrown if token refresh fails due to network issues or SoundCloud auth service being unavailable.
        /// </exception>
        public async Task<AuthenticationHeaderValue> NewBearerAuthenticationHeader( )
            => new( "Bearer", await GetAppTokenAsync( ) );

        /// <summary>
        /// Gets the client ID for direct use in API requests. SoundCloud API often uses client_id
        /// as a query parameter instead of or in addition to OAuth tokens.
        /// </summary>
        public string GetClientId( ) => auth.ClientId;
